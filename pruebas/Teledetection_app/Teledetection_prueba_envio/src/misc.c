...
// {
//     unsigned** medians = malloc(sizeof(unsigned*) * NB_CLASS); //cambiar
//     for (size_t i = 0; i < NB_CLASS; i++)
//         medians[i] = 0;
...
// unsigned* init_centers(unsigned centers[NB_CLASS])
// {
//   ; //= malloc(sizeof(unsigned) * NB_CLASS); //cambiar
...
void init_pixels(unsigned pixels[NB_PIXELS][VEC_SIZE], ImageFormat fmt, uint8_t* img)
{
  ...
  for (size_t x = 1; x < nb_col-1; x++)
  {
    for (size_t y = 1; y < nb_line-1; y++)
    {
      ...
      //pixels[pos] = malloc(sizeof(unsigned) * VEC_SIZE); //cambiar
      //pixels[pos][VEC_SIZE];
  ...
  for (size_t x = 0; x < fmt.nb_col; x++)
  {
      for (size_t y = 0; y < fmt.nb_line; y++)
      {
          ...
          //for (size_t chan = 0; chan < fmt.nb_chan; chan++)
            v += img_dst[pos+0];//img_src[pos+chan];
            v += img_dst[pos+1];
            v += img_dst[pos+2];
          v /= fmt.nb_chan;
